...
1. Jenis observer pattern yang digunakan adalah push. Hal ini dapat dilihat dari NotificationService yang secara aktif memberikan notifikasi kepada seluruh subscriber jika terjadi update dalam product tertentu, dan masing-masing subscriber yang tidak melakukan request data secara aktif untuk menerima notifikasi.

2. Kelebihan dari menggunakan observer pattern pull adalah request data yang dilakukan secara aktif dan periodik oleh masing-masing subscriber. Hal ini memastikan bahwa data yang dimiliki subscriber tidak mungkin lebih tua (old) dari periode request, sehingga tidak timbul rasa khawatir di benak pengguna seperti: "Data ini terakhir diupdate 16 hari yang lalu, apa benar sampai sekarang belum ada perubahan? Atau jangan-jangan ada yang miss?" Namun, kelebihan ini sekaligus dapat menjadi kekurangan bagi variasi pull, karena request data oleh subscriber yang bersifat periodik mungkin tidak efisien dan malah menambah beban aplikasi -- Terjadi request yang unecessary karena memang tidak ada data yang diupdate. Apalagi jika jumlah subscriber sangat banyak dan periode antar request sangat singkat, tentunya akan mengurangi kinerja aplikasi.

3. Jika multithreading tidak digunakan, maka akan terjadi overhead waktu yang sangat tinggi saat menghandle proses notifikasi. Misal waktu notifikasi 1 detik, dan terdapat 500 subscriber yang terhubung dengan satu produk. Jika terjadi update pada produk tersebut dan implementasi menggunakan singlethreading, maka akan ada antrian selama 500 detik untuk menyelesaikan proses tersebut, sehingga user di antrian terakhir membutuhkan 500 detik untuk mendapatkan notifikasi. Jika multithreading digunakan, masing-masing proses notifikasi berjalan secara paralel, sehingga setiap user membutuhkan 1 detik untuk mendapatkan notifikasi (tentu dengan asumsi 1 proses 1 thread, implementasi yang tidak realistis di lapangan).
